...
export class BaseApiService {
    ...
    public url:string = '';
    public newMsg:string = 'Registro creado con exito';
    public editMsg:string = 'Registro actualizado con exito';
    public deleteMsg:string = 'Registro eliminado con exito';
    public msg_time:number = 5000;

    public formErrorMsg:string = 'Oops! Cambia algunas cosas e intenta nuevamente.';    
  ...
  getPostData(parameters:any = []):  Observable<any> { 
    ...
    this.loading = true;      
    return this.http.post<any>(this.url, parameters).pipe(    
        catchError((error) => {
            console.log(error)
            return this.error(error);                                
        }),         
	...
	save(model:any,type:string = 'object'):  Observable<any> {
		...
		if(!id){     
			...
			var request = this.http['post']<any>(this.url, model);
        }else{
            var msgSuccess = this.editMsg;
            var request =  this.http[put_method]<any>(this.url+id, model);
        }		
    ...
    delete(id:any){
        this.loading = true; 
        var request =  this.http['delete']<any>(this.url+id);
        ...
        return request.pipe( 
            ...
            map((response) => {
                this.snackBar.open(this.deleteMsg, 'X', {
                    duration: this.msg_time,
                    panelClass: []
                });
